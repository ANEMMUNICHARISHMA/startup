  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center p-6 text-center">
      ...
      <div className="relative mb-6 animate-bounce" style={{ animationDuration: '3s' }}>
        ...
        <div className="relative flex items-center justify-center w-20 h-20 rounded-2xl bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 shadow-lg text-blue-600 dark:text-blue-400">
          <AlertOctagon className="w-10 h-10" />
        </div>
      </div>

      {/* Typography headers explaining the 404 code state */}
      <h1 className="font-display font-extrabold text-5xl md:text-6xl text-slate-900 dark:text-slate-50 tracking-tight">
        404
      </h1>
      
      <h2 className="mt-2 font-display font-bold text-lg md:text-xl text-slate-800 dark:text-slate-200">
        Page Not Found
      </h2>

      <p className="mt-3 text-xs md:text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto leading-relaxed">
      ...
      <div className="mt-8 w-full max-w-xs relative">
        ...
        <input
          ...
          className="w-full h-9 pl-9 pr-4 text-xs font-semibold text-slate-600 dark:text-slate-350 bg-slate-100 hover:bg-slate-250 dark:bg-slate-900/60 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg cursor-pointer text-left outline-none transition-all"
          onClick={() => navigate('/')}
        />
      </div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
        ...
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-1.5 h-9 px-4 text-xs font-semibold text-slate-650 hover:text-slate-950 dark:text-slate-350 dark:hover:text-slate-100 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/40 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-lg cursor-pointer transition-all"
        >
